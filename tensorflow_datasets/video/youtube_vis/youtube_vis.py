...
def _decode_segmentation(segmentation: Union[List[NestedDict], NestedDict],
  ...
  rle = mask.frPyObjects(segmentation, video['height'], video['width'])
  if isinstance(segmentation, list):  # Polygon
    rle = mask.merge(rle)
  segmentation = mask.decode(rle)
...
def _create_per_track_annotation(
  ...
  """
  anno = track_annotation
  assert video['id'] == anno['video_id']
  height = desired_height or video['height']
  width = desired_width or video['width']
  per_track_anno = {}

  per_track_anno['bboxes'] = []  # List of bounding box for the track.
  per_track_anno['frames'] = []  # List of frames the track appears on.
  frames_with_labels = []
  for frame_idx, box in enumerate(anno['bboxes']):
    if box is None:
      continue
  ...
  per_track_anno['segmentations'] = []  # List of segmentations for the track.
  for frame_idx, segmentation in enumerate(anno['segmentations']):
  ...
  for frame_idx, area in enumerate(anno['areas']):
    if area is None:
      assert frame_idx not in frames_with_labels
      continue
  ...
  per_track_anno['category'] = anno['category_id'] - 1
  per_track_anno['is_crowd'] = anno['iscrowd']
...
class YoutubeVisConfig(tfds.core.BuilderConfig):
  ...
  def __init__(self,
               *,
               ...
               only_frames_with_labels: Optional[int] = False,
               **kwargs):
    ...
    Args:
      ...
      only_frames_with_labels: optional width to resize all images to
...
class YoutubeVis(tfds.core.BeamBasedBuilder):
  ...
  Please download all files (test_all_frames.zip, test.json,
  train_all_frames.zip, train.json, valid_all_frames.zip, valid.json) from the
  youtube-vis webset and move them to ~/tensorflow_datasets/downloads/manual/.
  """
  BUILDER_CONFIGS = [
      YoutubeVisConfig(
          name='full',
